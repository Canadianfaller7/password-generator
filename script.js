...
const writePassword = () => {
  const passLength = generatePassword()
  const passCriteria = passwordCriteria()
  let password = randomizePassword(passLength, passCriteria);
...
const randomizePassword = (passLength, passCriteria) => {

  let randomPassword = []
  
  for (let i = 0; i < passLength; i++) {
    let newPassword = passCriteria[Math.floor(Math.random() * passCriteria.length - 1)]
...
/* 
  ...
  4. Take the information that was joined into 1 array and then run it through a for loop and then randomize the array to the length of what the user put in and then return it and pass it into our password variable 
